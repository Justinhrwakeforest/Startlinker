...
def admin_user_action(request):
    ...
    # Debug logging removed to fix OSError on Windows
        ...
        if report_id:
            ...
            try:
                ...
                if user_id:
                    ...
                    # Target user determined
            except UserReport.DoesNotExist:
                # UserReport not found, trying PostReport
                try:
    ...
    with transaction.atomic():
        if action_type == 'warn':
            ...
            if existing_ban:
                # Check if admin wants to extend existing ban
                extend_ban = request.data.get('extend_existing_ban', False)
                if extend_ban and existing_ban.ban_type == 'temporary':
                    # Extend the existing ban
                    from datetime import timedelta
                    additional_days = timedelta(days=duration)
                    if existing_ban.expires_at:
                        existing_ban.expires_at += additional_days
                    else:
                        existing_ban.expires_at = timezone.now() + additional_days
                    existing_ban.reason += f" | Extended by {duration} days: {reason}"
                    existing_ban.save()
                    ...
                    Notification.create_notification(
                        recipient=target_user,
                        notification_type='system',
                        title='⚠️ Ban Extended',
                        message=f"Your temporary ban has been extended by {duration} days. New expiry: {existing_ban.expires_at.strftime('%Y-%m-%d %H:%M:%S')}. Reason: {reason}",
                        sender=None,
                        extra_data={
                            'ban_id': existing_ban.id,
                            'admin_username': request.user.username,
                            'action_type': 'ban_extension',
                            'additional_days': duration,
                            'new_expires_at': existing_ban.expires_at.isoformat()
                        }
                    )
                    
                    return Response({
                        'success': True,
                        'message': f'Temporary ban extended by {duration} days',
                        'ban_id': existing_ban.id,
                        'new_expiry': existing_ban.expires_at.isoformat(),
                        'action_type': 'ban_extended'
                    })
                else:
                    # Provide detailed error with options
                    ban_info = {
                        'error': f'User already has an active {existing_ban.get_ban_type_display().lower()}',
                        'existing_ban': {
                            'id': existing_ban.id,
                            'type': existing_ban.ban_type,
                            'created_at': existing_ban.created_at.isoformat(),
                            'expires_at': existing_ban.expires_at.isoformat() if existing_ban.expires_at else None,
                            'reason': existing_ban.reason,
                            'admin': existing_ban.admin.username if existing_ban.admin else 'System'
                        },
                        'options': {
                            'extend_ban': existing_ban.ban_type == 'temporary',
                            'message': 'Set extend_existing_ban=true to extend the current temporary ban' if existing_ban.ban_type == 'temporary' else 'Cannot extend permanent bans'
                        }
                    }
                    return Response(ban_info, status=status.HTTP_400_BAD_REQUEST)
            ...
            existing_ban = UserBan.objects.filter(user=target_user, is_active=True).first()
            if existing_ban:
                # Provide detailed error for permanent ban attempts
                ban_info = {
                    'error': f'User already has an active {existing_ban.get_ban_type_display().lower()}',
                    'existing_ban': {
                        'id': existing_ban.id,
                    ...
                    'options': {
                        'upgrade_to_permanent': existing_ban.ban_type == 'temporary',
                        'message': 'Cannot apply permanent ban - user already has an active ban. Consider waiting for temporary ban to expire.' if existing_ban.ban_type == 'temporary' else 'User already permanently banned.'
                    }
                }
                return Response(ban_info, status=status.HTTP_400_BAD_REQUEST)
...
class ReportListView(generics.ListAPIView):
    """
    List all reports for authenticated users (non-admin access restricted)
    """
    permission_classes = [permissions.IsAuthenticated]
    pagination_class = ReportPagination
    ...
    def get_queryset(self):
        ...
        # Only allow admin users to see all reports
        if not user.is_staff and not user.is_superuser:
            # Return empty queryset for non-admin users
            return UserReport.objects.none()
        
        # Admin users can see all reports
        return UserReport.objects.select_related('reported_user', 'reporter').all()
    
    def get_serializer_class(self):
        if self.request.user.is_staff or self.request.user.is_superuser:
            return AdminUserReportSerializer

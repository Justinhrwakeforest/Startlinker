...
class SuggestedUsersViewSet(viewsets.ViewSet):
    """ViewSet for suggested users to follow"""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def list(self, request):
        ...
        # Get suggested users based on activity and popularity
        ...
        suggested_users = User.objects.filter(
            ...
            follower_count_real=Count('follower_relationships', distinct=True),
            posts_count=Count('posts', distinct=True)
        ).order_by('-follower_count_real', '-posts_count', '-date_joined')[:20]
